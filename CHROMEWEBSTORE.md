...
- 可从已构建的本地仓库启动 dsh；此启动方式暂不加载 profile 中的 SSH 连接插件
- 搭配新版配套插件，服务主动报告就绪状态和实际端口，减少页面认证变化对状态识别的影响
...
- 不读取、不存储 dsh 的凭据文件。
- 配套插件新增仅本机的就绪文件，记录启动标识、PID、端口、时间与版本；不含会话内容或认证 token，正常卸载时删除。不新增扩展权限。
...
|---------|------|---------|--------|
| 0.3.0 | 2026-09-26 | Whalekeeper 更名、会话深链、实时会话推送、新版 dsh 启动认证兼容、本地源码启动和主动就绪报告；修复带令牌的会话深链，加快运行状态获取 | Draft |
| 未发布（本地源码启动） | 2026-09-23 | 本地仓库启动时临时跳过 SSH 连接插件，避免它阻塞 Web 界面就绪；全局和 NPX 启动保持原行为 | Draft |
| 未发布（配套插件 0.4.0） | 2026-09-21 | 主动就绪与动态端口报告，旧插件自动回退；扩展权限与界面不变 | Draft |
