...
### Added
- **M10 颜色语义自定义（2026-08-24，design §8.12；UI 区名「颜色角色」，术语=语义色 semantic color）**：设置「界面」分组新增「颜色角色」区——
  `waiting/done/working/completed/idle` 五角色每行「语义标签 + 当前色点 + 6 个预设圆形 swatch
  （蓝/琥珀/紫/绿/红/灰）」点选即生效（同 theme-cube 交互 + 行内方向键导航）+「恢复默认色板」；
  保存其它设置不覆盖。`settings.colorMap` 存 storage（`DEFAULT_SETTINGS` 声明默认值），
  **error 红（#ec1313）与字符/文字语义锁定**（§8.9.1 硬规则），改「等你拍板/完成待办/已完成」
  ...
  popup 会话区四态圆点走语义变量（`.dot-error` 集中引用锁定变量）→ **SW 徽标底色运行时读
  colorMap**（`background.js` `normColorMap` 零依赖同口径；字符 `?`/`!`/n 恒在，底色随配置）。
  **working 双载体默认色统一为 webui 蓝 #5686fe**（徽标原值、popup 会话区由琥珀改蓝——仍为
  **提案值**，待用户实际体验调色后定稿回填，可随时改回琥珀）。**生效范围取舍（§8.12 实施注记）**：
  colorMap 严格按角色表载体列生效（徽标 + popup 会话区圆点）；状态展示层（状态卡/面板/logs
  实例圆点）不随角色色改——其主语义是颜色（§8.9.1），防「空闲→绿」把已停止实例渲染成绿点
  毁坏实例语义，继续走 `--dsw-alias-state-*` 令牌。验收：verify-cdp **92 / FAIL 0**（新增
  8 条：默认色板四态、改色实际变化、实例层锁定、撞色 toast、恢复默认（storage 键级比对）、
  字符语义回归、徽标底色随 colorMap、徽标恢复默认）；`node --check` 全过；无新增 console 异常。
...
### Changed
- **M9 会话区「进行中」圆点默认色由琥珀改蓝（M10 统一，2026-08-24）**：§8.12 working 双载体
  默认色统一为 webui 蓝 `#5686fe`——popup 会话区 `sdot-working` 由 `--dsw-alias-state-warn-label`
  （琥珀）改为 `--dsh-mgr-sem-working`（默认 webui 蓝，**提案值**，用户可经「颜色角色」区改回琥珀）；
  徽标蓝 n 原值不变；verify-cdp M9 段圆点色断言同步更新。
