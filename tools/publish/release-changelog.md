...
- 页面内管理面板：扩展重载/更新后，已打开页面里的旧面板不再永久红错「状态获取失败」——
  显示中性灰提示「扩展已重载或更新，请刷新页面恢复」（徽章「已断开」）并停止轮询；
  单次状态获取失败不再立即变红（连续 3 次约 6s 才显示错误态，期间保留上次成功状态）
- 圆点颜色改随展示语义：绿=本页托管实例运行中、蓝=外部实例、灰=未托管/已停止、
  琥珀=启动/停止中、红=连续失败（此前「未托管」页面误显绿灯）
