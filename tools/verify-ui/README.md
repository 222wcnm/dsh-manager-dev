...
| 脚本 | 原理 | 适用环境 | 现状 |
|------|------|----------|------|
| `verify-cdp.js` | 零依赖 CDP 直连（headless Chrome + `Extensions.loadUnpacked` + Page.captureScreenshot），与 `tools/icons/_gen-icons.js` 同手法 | **沙箱内可用（已实测）** | 最小验收 9/9 通过（扩展加载、popup/日志页截图 + 文本断言 + console 异常检查） |
