...
- `docs/design.md` — 唯一规格：架构、宿主协议（§6）、安全模型（§12）、路线图（§15）；事实基线（§2，按本机 `@deepseek-ai/dsh@0.1.1-rc.2` 核验）；**上游破坏性变更台账（§2.1）——dsh 版本跳变时先查此表，勿重复调查**
...
node native-host/test/smoke.js                       # 冒烟测试 29 场景（M13 后本机实测：Windows 372 PASS+1 SKIP——场景 26 POSIX；历史口径：Windows 347、Linux 337、全新克隆 332+4 SKIP——3 项本地产物核对跳过；BASE_ENV 进程枚举围栏仅 Windows 生效——场景 26 在 POSIX 不注入任何钩子走真实 /proc 平台层）
...
node native-host/test/e2e-rc1-isolated.js            # M13 真实 rc.1 隔离 e2e（临时前缀 @deepseek-ai/dsh@0.1.2-rc.1 + 本地插件方式 B 装配；DSH_MANAGER_NPM_PREFIX 可覆盖前缀，缺省 %TEMP%\dsh-rc1-prefix；2026-09-04 实跑 9/9 PASS）
node --test "plugin\dsh-lifecycle\test\*.test.js"    # dsh-lifecycle 插件单测（52 项，M13 起）
...
node tools/verify-ui/verify-cdp.js                   # 扩展 UI 自动验收 107 断言（沙箱内可用，若沙箱拦 headless Chrome 启动则需沙箱外；M13 起：真实 rc.1 实例先做认证引导——探测 401 → 读 run 记录 launchUrl → token 兑换 cookie，面板注入/SSE/主题面板段才可达；popup/logs/面板注入与展开 + 面板停止两步确认态（首击确认/3s 还原，不执行） + 面板体验回归（托管绿点+端口、展开时胶囊位置不变、面板在胶囊上方、扩展重载后旧面板提示刷新/刷新恢复） + 日志页「加载更早/复制全部」交互 + popup 设置校验交互 + 徽标三步实测（M13：测试期 attention:false 隔离真实工作会话干扰）+ M8/M8.1 徽标提醒（SW 分层 done「!」/ waiting「?」/ 蓝 n 工作中计数 / 9+ 边界 / done>working 优先级 / 清空恢复 + 角标断言（icon 状态 + title）+ e2e 后台页注入等待标记 → 黄「?」→ 切回标签自动清除）+ 主题与深色断言（面板深色跟随、storage 镜像写入、popup 四态 + Emulation 系统模拟、深色段无新增 console 异常）+ M7 断言（状态卡结构/文案、状态变体（running 实心点呼吸 / external / busy 琥珀脉冲无矩阵 / error 红调卡 / stopped 灰点）、深/浅状态卡背景（深=#1e2025 渲染）、面板呼吸动画）+ M9 断言（会话区初始隐藏、四态渲染+圆点色表（M11：待确认>进行中/已停止>已完成新鲜；idle 与陈旧完成不渲染）、title 降级「会话 #id 前8」、子代理树行、空态、插件不可用降级）+ M10 断言（默认色板三态、改色后会话区/徽标底色实际变化、实例层圆点不随角色色改、撞色 toast、恢复默认（storage 键级比对）、字符语义回归）+ M11 断言（仅新鲜完成显示、已读入口与 3s 撤销倒计时、落库移除行、retentionMins=0 时已停止恒显）+ M12 断言（SSE 合成帧：快照→徽标「1」即时、SSE 活跃 0 端点轮询、upsert→「?」即时、removed→恢复、working→空→done「!」、时序回归）+ M13 断言（openWebUI 有 launchUrl 用 launchUrl（深链 hash 保留）/无则回退裸 URL + 认证引导）+ console 异常检查；默认 dsh URL 3080（VERIFY_DSH_URL 覆盖））
...
- Node ≥ 18；dsh **0.1.2-rc.1** 全局安装（2026-09-04 升级，npm `latest` = rc.1）；默认端口 3080
- **上游 dsh 版本跟踪**：破坏性变更集中登记于 design §2.1 台账。**M13 一期（0.1.2 认证兼容 + 插件 Session.events 适配）已完成（2026-09-04，见顶部记录）**；当前待办 = **M13 二期**：插件写「就绪文件」把存活/端口判定由宿主轮询探测反转为 dsh 主动告知（§15.2，需先定协议：文件路径/新鲜度/双信号）。约束：**任何 dsh HTTP 探测一律不发 `Accept-Encoding`**（0.1.2 起 webserver 默认 gzip）。上游最新：**`v0.1.2-rc.1` 已发布 npm 并成为 `latest`/`next`**（2026-09-04 晚 `npm view dist-tags` 核验：`{latest: 0.1.2-rc.1, next: 0.1.2-rc.1, alpha: 0.1.2-alpha.5}`）；**本机已升级 `0.1.2-rc.1`**（2026-09-04），真实 profile 插件已同步更新为 M13 版（备份 `index.js.bak-pre-m13`），真机回归已跑通（smoke-real 全链路 + e2e-m9-manager 7/7，见顶部记录）
...
- **M13 上游 0.1.2 认证兼容 + 插件 Session.events 适配（2026-09-04，design §2.1.1/§6.2/§6.3/§8.10/§12.3）**：上游 0.1.2 系列已迭代到 **v0.1.2-rc.1**（GitHub release 2026-09-03，commit a66e470204；**npm 已发 `latest`/`next` = `0.1.2-rc.1`**，2026-09-04 晚核验）；本地 `D:\deepseek-harness` 已 checkout `dsh-v0.1.2-rc.1` 做逐项源码复核。**rc.1 关键事实**：① 浏览器启动令牌认证闸门**无回环豁免**（`GET /` 无凭据 401，127.0.0.1 同受保护）；② `/manifest.webmanifest` 为公开静态资产（含 `"name": "DeepSeek Harness"` 指纹）；③ **`Session.events` 数组属性移除**（改 `eventAt()`/`snapshotEvents()`/`ownEvents()` 按需读取，alpha.4 起、rc.1 确认）；④ 其余插件接缝（webServer.register/appExit/sessions/agents/workspaceRegistry/事件总线/agent/status `{agent,status}`）全部存活。**实施**：① 宿主 `httpProbe` 将 401 视为就绪、`httpDshProbe` 指纹端点改 `/manifest.webmanifest`（外部发现/PORT_BUSY 指纹/adopt 复查同源受益）；② 日志解析新增捕获完整 `dsh web: <url>` 行（含 token）→ run 记录与 status 新增 **`launchUrl`**（`url` 保持裸 URL；§12.3 边界：仅本机 run 记录存储、仅经 native 返回 SW 用于开标签、不入 storage/UI 文本）；③ 插件两处 `session.events` → `sessionEvents()` 版本兼容读取（`.events` 数组 → `snapshotEvents()` → 空数组三级回退），peerDeps 范围修正 `>=0.1.0-rc.6 <0.2.0-0`（§2.1.1 R2）；④ popup `openWebUI` 新建标签优先 `launchUrl`（深链 hash 在 303 重定向后保留），已有标签复用/跳转仍裸 URL；⑤ fake-dsh 增 `DSH_FAKE_AUTH=1`（401+manifest+token URL 行）与 `DSH_FAKE_LOG_HEADERS=1`（Accept-Encoding 打点）变体，smoke 新增**场景 29**（auth start→running、launchUrl 捕获、url 裸 URL、优雅停、无 Accept-Encoding、外部 auth 实例 manifest 发现、PORT_BUSY 提示），插件单测 +2（rc.1 形状主路径、双缺降级 idle），verify-cdp +2（openWebUI launchUrl 优先/裸 URL 回退，stub chrome.tabs）。**验证（2026-09-04 实跑）：插件单测 52/52；smoke 372 PASS / FAIL 0 / SKIP 1（场景 29 新增 9 断言全过）；verify-cdp 107 PASS / FAIL 0**（verify-cdp 顺带修复 M12 适配缺口——M8 端点 mock 在 SSE 存活环境不可达（面板 attTick 不轮询端点），两端点驱动分支改如实记录，waiting/done 徽标链路改由 M12 段合成帧断言覆盖，含新增 done「绿!」合成帧断言；**M13 认证引导**——真实 rc.1 实例裸 URL 401，verify 导航前先探测 401 → 读 run 记录 launchUrl → token 兑换 cookie（浏览器内 title 校验），面板注入/SSE/主题面板段才可达；徽标段测试期 attention:false 隔离真实工作会话干扰）。**真机回归（2026-09-04，本机已升级全局 rc.1）**：`smoke-real.js` 全链路通过（start→status→stop + `--port 0` 动态端口回填，launchUrl 捕获真实 token）；`e2e-m9-manager.js` **7/7 PASS**（真实 profile 插件装配：`/_manager/sessions` 200、围栏 403、非 GET 405、`/_manager/events` SSE 200+snapshot 帧、宿主 sessions available:true、lifecycle:true、stop graceful）；隔离前缀 `e2e-rc1-isolated.js` 9/9 PASS；**verify-cdp 对真实 rc.1 实例全量回归 107/0**。残余风险（低）：外部实例（无 run 记录 token）打开 UI 仍 401（回退裸 URL，上游安全模型固有限制，design 已标注）。
