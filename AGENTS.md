...
node tools/verify-ui/verify-cdp.js                   # 扩展 UI 自动验收 28 断言（沙箱内可用；popup/logs/面板注入与展开 + 面板停止两步确认态（首击确认/3s 还原，不执行） + 日志页「加载更早/复制全部」交互 + popup 设置校验交互 + 徽标三步实测 + console 异常检查）
